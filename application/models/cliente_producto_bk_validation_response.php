...
class Cliente_producto extends CI_Model
{
    ...
    public function add_cliente_producto_credito($clientesCreados, $value, $idEmpresa, $mes, $ano, $codigoAgencia, $idCreador)
    {
        ...
        if (! empty($asociado)) {
            if (! empty($producto)) {
                $clientesCreados = $this->create_cliente_producto_values($clientesCreados, 'credito', $asociado, $value['Valor_cuota_fija'], $value['Tasa_de_interes_nominal_cobrada'], $producto['intCodigo'], $codigoAgencia, $idCreador, $ano, $mes, $value['Fecha_de_desembolso_inicial']);
            } else {
